 ...
 * @LastEditors: kenan
 * @LastEditTime: 2022-09-13 15:13:56
	...
	if runtime.GOOS == "windows" && !isVmCommand {
		...
		if isVmCommand {
			if common.SmartIDELog.Ws_id == "" {
				configPaths = []string{fmt.Sprintf("$HOME/.ssh/id_rsa_%s_%s:/home/smartide/.ssh/id_rsa", userName, common.SmartIDELog.Ws_id), fmt.Sprintf("$HOME/.ssh/id_rsa.pub_%s_%s:/home/smartide/.ssh/id_rsa.pub", userName, common.SmartIDELog.Ws_id), fmt.Sprintf("$HOME/.ssh/authorized_keys_%s_%s:/home/smartide/.ssh/authorized_keys", userName, common.SmartIDELog.Ws_id), "$HOME/smartide-agent:/home/smartide/smartide-agent"}
			}
