...
| 版本号      | 构建编号 | 发布日期      |   简要说明   |
| ----------- | ----------- | ----------- | ----------- |
| v1.0.25 | 5383 | 2022.08.29 | 主要包括：11.工作区SSH策略、密码策略支持K8S工作区；2.VMLC开发者镜像调整；
3.smartide init项目初始化指令；4.ide.yaml同时支持docker-compose及k8s模式；5.优化日志显示，独立显示ingress及ssh日志信息；6.Server部署数据持久化；7.k8s basic密码使用提示；8.修复若干Bug |
